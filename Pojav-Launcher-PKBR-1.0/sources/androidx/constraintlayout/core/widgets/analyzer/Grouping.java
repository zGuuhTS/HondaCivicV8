...
public class Grouping {
    ...
    public static boolean simpleSolvingPass(ConstraintWidgetContainer layout, BasicMeasure.Measurer measurer) {
        ...
        ConstraintAnchor top2 = constraintWidgetContainer.getAnchor(ConstraintAnchor.Type.TOP);
        if (top2.getDependents() != null) {
            Iterator<ConstraintAnchor> it9 = top2.getDependents().iterator();
            while (it9.hasNext()) {
    ...
    public static WidgetGroup findDependents(ConstraintWidget constraintWidget, int orientation, ArrayList<WidgetGroup> list, WidgetGroup group) {
        ...
        if (groupId != -1 && (group == null || groupId != group.f66id)) {
            int i = 0;
            while (true) {
                if (i >= list.size()) {
                    break;
