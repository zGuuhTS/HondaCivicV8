...
public class JsonObject extends JsonElement implements Map<String, JsonElement>
{
    ...
    public String toJson(final boolean comments, final boolean newlines, final int depth, final int newLinesAmount) {
        final JsonGrammar grammar = JsonGrammar.builder().withComments(comments).printWhitespace(newlines, newLinesAmount).build();
        return this.toJson(grammar, depth);
    }
    ...
    public void toJson(final Writer w, final JsonGrammar grammar, final int depth) throws IOException {
        ...
        for (int i = 0; i < this.entries.size(); ++i) {
            ...
            if (grammar.shouldOutputWhitespace()) {
                if (i > 0 && entry.getComment() != null && !entry.getComment().trim().isEmpty()) {
                    for (int j = 1; j < grammar.getNewLinesAmount(); ++j) {
                        w.append('\n');
                    }
                }
