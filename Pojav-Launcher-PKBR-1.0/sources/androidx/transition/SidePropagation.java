...
public class SidePropagation extends VisibilityPropagation {
    ...
    public long getStartDelay(ViewGroup sceneRoot, Transition transition, TransitionValues startValues, TransitionValues endValues) {
        ...
        int top2 = loc[1] + Math.round(sceneRoot.getTranslationY());
        int right = left + sceneRoot.getWidth();
        int bottom = top2 + sceneRoot.getHeight();
        if (epicenter != null) {
            epicenterX = epicenter.centerX();
            epicenterY = epicenter.centerY();
        } else {
            epicenterY = (top2 + bottom) / 2;
            epicenterX = (left + right) / 2;
        }
        int[] iArr = loc;
        TransitionValues transitionValues2 = positionValues;
        float distanceFraction = ((float) distance(sceneRoot, viewCenterX, viewCenterY, epicenterX, epicenterY, left, top2, right, bottom)) / ((float) getMaxDistance(sceneRoot));
    ...
    private int distance(View sceneRoot, int viewX, int viewY, int epicenterX, int epicenterY, int left, int top2, int right, int bottom) {
        int side;
        int i = this.mSide;
        int i2 = 5;
        boolean isRtl = false;
        ...
        switch (side) {
            ...
            case 80:
                return (viewY - top2) + Math.abs(epicenterX - viewX);
            default:
                return 0;
        }
    }
