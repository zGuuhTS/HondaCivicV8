...
public class Debug {
    ...
    public static String getState(MotionLayout layout, int stateId, int len) {
        ...
        if (str.length() <= len || (n = str.replaceAll("[^_]", BuildConfig.FLAVOR).length()) <= 0) {
            return str;
        }
        return str.replaceAll(CharBuffer.allocate((str.length() - len) / n).toString().replace(0, FilenameUtils.EXTENSION_SEPARATOR) + "_", "_");
    }
