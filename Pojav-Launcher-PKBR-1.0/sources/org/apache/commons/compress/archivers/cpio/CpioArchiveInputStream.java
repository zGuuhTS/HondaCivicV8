...
public class CpioArchiveInputStream extends ArchiveInputStream implements CpioConstants {
    ...
    /* renamed from: in */
    private final InputStream f139in;
    ...
    private final int readFully(byte[] bArr, int i, int i2) throws IOException {
        int readFully = IOUtils.readFully(this.f139in, bArr, i, i2);
        count(readFully);
        if (readFully >= i2) {
            return readFully;
        }
    ...
    private CpioArchiveEntry readNewEntry(boolean z) throws IOException {
        CpioArchiveEntry cpioArchiveEntry = z ? new CpioArchiveEntry(2) : new CpioArchiveEntry(1);
    ...
    public CpioArchiveEntry getNextCPIOEntry() throws IOException {
        CpioArchiveEntry readOldAsciiEntry;
        ensureOpen();
        if (this.entry != null) {
            closeEntry();
        }
        ...
        if (CpioUtil.byteArray2long(this.TWO_BYTES_BUF, false) == 29127) {
            readOldAsciiEntry = readOldBinaryEntry(false);
        } else if (CpioUtil.byteArray2long(this.TWO_BYTES_BUF, true) == 29127) {
            readOldAsciiEntry = readOldBinaryEntry(true);
        } else {
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        if (this.entry.getFormat() == 2) {
            for (int i3 = 0; i3 < readFully; i3++) {
                this.crc += (long) (bArr[i3] & UByte.MAX_VALUE);
            }
        }
