...
public class Motion implements TypedValues {
    ...
    public float getStartX() {
        return this.mStartMotionPath.f37x;
    }

    public float getStartY() {
        return this.mStartMotionPath.f38y;
    }

    public float getFinalX() {
        return this.mEndMotionPath.f37x;
    }

    public float getFinalY() {
        return this.mEndMotionPath.f38y;
    }
    ...
    public MotionKeyPosition getPositionKeyframe(int layoutWidth, int layoutHeight, float x, float y) {
        ...
        start.left = this.mStartMotionPath.f37x;
        start.f208top = this.mStartMotionPath.f38y;
        start.right = start.left + this.mStartMotionPath.width;
        start.bottom = start.f208top + this.mStartMotionPath.height;
        FloatRect end = new FloatRect();
        end.left = this.mEndMotionPath.f37x;
        end.f208top = this.mEndMotionPath.f38y;
        end.right = end.left + this.mEndMotionPath.width;
        end.bottom = end.f208top + this.mEndMotionPath.height;
    ...
    public float getKeyFrameParameter(int type, float x, float y) {
        float dx = this.mEndMotionPath.f37x - this.mStartMotionPath.f37x;
        float dy = this.mEndMotionPath.f38y - this.mStartMotionPath.f38y;
        float startCenterX = this.mStartMotionPath.f37x + (this.mStartMotionPath.width / 2.0f);
        float startCenterY = this.mStartMotionPath.f38y + (this.mStartMotionPath.height / 2.0f);
    ...
    public String toString() {
        return " start: x: " + this.mStartMotionPath.f37x + " y: " + this.mStartMotionPath.f38y + " end: x: " + this.mEndMotionPath.f37x + " y: " + this.mEndMotionPath.f38y;
    }
    ...
    public void setStartState(ViewState rect, MotionWidget v, int rotation, int preWidth, int preHeight) {
        ...
        switch (rotation) {
            case 1:
                ...
                r.left = ((rect.f210top + rect.bottom) - rect.width()) / 2;
                r.f209top = preWidth - ((rect.height() + cx) / 2);
                r.right = r.left + rect.width();
                r.bottom = r.f209top + rect.height();
                break;
            case 2:
                int cx2 = rect.left + rect.right;
                r.left = preHeight - ((rect.width() + (rect.f210top + rect.bottom)) / 2);
                r.f209top = (cx2 - rect.height()) / 2;
                r.right = r.left + rect.width();
                r.bottom = r.f209top + rect.height();
                break;
        }
        this.mStartMotionPath.setBounds((float) r.left, (float) r.f209top, (float) r.width(), (float) r.height());
    ...
    public void rotate(Rect rect, Rect out, int rotation, int preHeight, int preWidth) {
        switch (rotation) {
            case 1:
                int cx = rect.left + rect.right;
                out.left = ((rect.f209top + rect.bottom) - rect.width()) / 2;
                out.f209top = preWidth - ((rect.height() + cx) / 2);
                out.right = out.left + rect.width();
                out.bottom = out.f209top + rect.height();
                return;
            case 2:
                int cx2 = rect.left + rect.right;
                out.left = preHeight - ((rect.width() + (rect.f209top + rect.bottom)) / 2);
                out.f209top = (cx2 - rect.height()) / 2;
                out.right = out.left + rect.width();
                out.bottom = out.f209top + rect.height();
                return;
            case 3:
                int cx3 = rect.left + rect.right;
                int i = rect.f209top + rect.bottom;
                out.left = ((rect.height() / 2) + rect.f209top) - (cx3 / 2);
                out.f209top = preWidth - ((rect.height() + cx3) / 2);
                out.right = out.left + rect.width();
                out.bottom = out.f209top + rect.height();
                return;
            case 4:
                int cx4 = rect.left + rect.right;
                out.left = preHeight - ((rect.width() + (rect.bottom + rect.f209top)) / 2);
                out.f209top = (cx4 - rect.height()) / 2;
                out.right = out.left + rect.width();
                out.bottom = out.f209top + rect.height();
                return;
            default:
            ...
            case -1:
                ...
                return new DifferentialInterpolator() {
                    ...
                    public float getVelocity() {
                        return (float) easing.getDiff((double) this.f34mX);
                    }
                };
            default:
                return null;
    ...
    public boolean interpolate(MotionWidget child, float global_position, long time, KeyCache keyCache) {
        ...
        if (curveFitArr != null) {
            ...
            float float_l = this.mStartMotionPath.f37x + ((this.mEndMotionPath.f37x - this.mStartMotionPath.f37x) * position);
            float float_t = this.mStartMotionPath.f38y + ((this.mEndMotionPath.f38y - this.mStartMotionPath.f38y) * position);
    ...
    public void getDpDt(float position, float locationX, float locationY, float[] mAnchorDpDt) {
        ...
        float dleft = this.mEndMotionPath.f37x - this.mStartMotionPath.f37x;
        float dTop = this.mEndMotionPath.f38y - this.mStartMotionPath.f38y;
