...
public class TarArchiveOutputStream extends ArchiveOutputStream {
    ...
    public void putArchiveEntry(ArchiveEntry archiveEntry) throws IOException {
        if (!this.finished) {
            ...
            tarArchiveEntry.writeEntryHeader(bArr, zipEncoding2, z);
            writeRecord(this.recordBuf);
            long j = 0;
            this.currBytes = 0;
            if (!tarArchiveEntry.isDirectory()) {
                j = tarArchiveEntry.getSize();
            }
            this.currSize = j;
            this.currName = name;
    ...
    public void writePaxHeaders(TarArchiveEntry tarArchiveEntry, String str, Map<String, String> map) throws IOException {
        ...
        for (Map.Entry next : map.entrySet()) {
            ...
            String str5 = length + " " + str3 + "=" + str4 + IOUtils.LINE_SEPARATOR_UNIX;
            int length2 = str5.getBytes("UTF-8").length;
            while (length != length2) {
                str5 = length2 + " " + str3 + "=" + str4 + IOUtils.LINE_SEPARATOR_UNIX;
                int i = length2;
