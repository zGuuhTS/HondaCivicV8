...
{
    public RemovePointsCommand(final BVRTournaments plugin) {
        super(plugin);
    }
    ...
    public CommandResult execute(final CommandSource src, final CommandContext args) {
        ...
        this.plugin.getDatabaseController().getUser(player).whenComplete((user, throwable) -> {
            if (user == null) {
                ...
                pointsToRemove = args.getOne("points").orElse(-1);
                if (pointsToRemove <= 0) {
                    ...
                    if (onlinePlayer.isPresent()) {
                        this.plugin.getDatabaseController().setPoints(onlinePlayer.get(), points - pointsToRemove);
                        if (this.plugin.getConfig().general.pointsWarnMessage) {
                            TextUtils.sendFormattedMessage(onlinePlayer.get(), this.plugin.getLang().geral.removedPointsMessage, pointsToRemove);
                        }
                    }
                    else {
                        this.plugin.getDatabaseController().setPoints(player2, points - pointsToRemove);
                    }
                    src.sendMessage(TextUtils.formatText(this.plugin.getLang().commands.pointsRemove, pointsToRemove));
                    return;
                }
            }
        });
