...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public NestedScrollView(Context context, AttributeSet attrs) {
        this(context, attrs, C0057R.attr.nestedScrollViewStyle);
    }
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        int i = AWTInputEvent.VK_DEAD_CIRCUMFLEX;
        if (!canScroll) {
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, AWTInputEvent.VK_DEAD_CIRCUMFLEX);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(AWTInputEvent.VK_DEAD_CIRCUMFLEX)) {
                return false;
            }
            return true;
            ...
            switch (event.getKeyCode()) {
                ...
                case 20:
                    if (!event.isAltPressed()) {
                        return arrowScroll(AWTInputEvent.VK_DEAD_CIRCUMFLEX);
                    }
                    return fullScroll(AWTInputEvent.VK_DEAD_CIRCUMFLEX);
                case 62:
    ...
    private View findFocusableViewInBounds(boolean topFocus, int top2, int bottom) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (top2 < viewBottom && viewTop < bottom) {
                boolean viewIsCloserToBoundary = false;
                boolean viewIsFullyContained = top2 < viewTop && viewBottom < bottom;
    ...
    private boolean scrollAndFocus(int direction, int top2, int bottom) {
        boolean handled = true;
        int height = getHeight();
        int containerTop = getScrollY();
        int containerBottom = containerTop + height;
        boolean up = direction == 33;
        View newFocused = findFocusableViewInBounds(up, top2, bottom);
        if (newFocused == null) {
            newFocused = this;
        }
        if (top2 < containerTop || bottom > containerBottom) {
            doScrollY(up ? top2 - containerTop : bottom - containerBottom);
        } else {
            handled = false;
        }
    ...
    public boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = AWTInputEvent.VK_DEAD_CIRCUMFLEX;
        } else if (direction == 1) {
            direction = 33;
        }
