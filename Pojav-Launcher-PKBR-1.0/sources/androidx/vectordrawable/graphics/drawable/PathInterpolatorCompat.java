...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (numPoints > 0) {
            this.f99mX = new float[numPoints];
            this.f100mY = new float[numPoints];
            float[] position = new float[2];
            for (int i = 0; i < numPoints; i++) {
                pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, (float[]) null);
                this.f99mX[i] = position[0];
                this.f100mY[i] = position[1];
            }
            if (((double) Math.abs(this.f99mX[0])) > 1.0E-5d || ((double) Math.abs(this.f100mY[0])) > 1.0E-5d || ((double) Math.abs(this.f99mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f100mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
                throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f99mX[0] + "," + this.f100mY[0] + " end:" + this.f99mX[numPoints - 1] + "," + this.f100mY[numPoints - 1]);
            }
            float prevX = 0.0f;
            int componentIndex = 0;
            int i2 = 0;
            while (i2 < numPoints) {
                float[] fArr = this.f99mX;
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f99mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f99mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        ...
        if (xRange == 0.0f) {
            return this.f100mY[startIndex];
        }
        float[] fArr2 = this.f100mY;
